...
namespace Disassembler.Formats.NE
{
	public class NEExecutable
	{
		...
		private List<NESegment> aSegments = new List<NESegment>();
		...
		private List<NEEntry> aEntries = new List<NEEntry>();
		...
		public NEExecutable(Stream stream)
		{
			...
			NEContentFlagsEnum eContentFlags = (NEContentFlagsEnum)ReadUInt16(stream);
			...
			{
				this.aSegments.Add(new NESegment(stream, iLogicalSectorSize));
			}
				...
				else
				{
					for (int i = 0; i < iCount; i++)
					{
						...
						if (iType < 0xff)
						{
							...
							this.aEntries.Add(new NEEntry(iType, sName, iFlag, iOffset));
						}
						else
						{
							int iFlag = ReadByte(stream1);
							...
							int iOffset = ReadUInt16(stream1);
							this.aEntries.Add(new NEEntry(iType, sName, iFlag, iInt3F, iSegment, iOffset));
						}
						iIndex++;
					}
				}
		...
		public List<NESegment> Segments
		{
			get
			{
		...
		public void ApplyRelocations()
		{
			...
			{
				NESegment segment = this.aSegments[i];
				bool bDataSegment = (segment.Flags & NESegmentFlagsEnum.DataSegment) == NESegmentFlagsEnum.DataSegment;
				...
				if (segment.Relocations.Count > 0)
				{
					...
					{
						NERelocation relocation = segment.Relocations[j];

						switch (relocation.RelocationType)
						{
							case NERelocationTypeEnum.InternalReference:
								...
								break;
							case NERelocationTypeEnum.ImportedOrdinal:
								...
								break;
							case NERelocationTypeEnum.Additive:
								// relocation.Parameter1, which is segment, is one (1) based
								switch (relocation.LocationType)
								{
									case LocationTypeEnum.Segment16:
								...
								break;
							case NERelocationTypeEnum.FPFixup:
								// ignore those
								break;
							case NERelocationTypeEnum.ImportedName:
							case NERelocationTypeEnum.OSFixup:
